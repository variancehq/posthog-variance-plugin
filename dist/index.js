...
async function onEvent(event, { config }) {
    ...
    const eventName = lodash_get(event, `event`);
    const { type, mapping } = eventName in eventToMapping
        ? eventToMapping[eventName]
        : eventToMapping.default;
    lodash_set(variancePayload, `type`, type);
    constructPayload(variancePayload, event, mapping);
    const properties = event.properties;
    if (properties) {
        Object.keys(properties).forEach((propKey) => {
            if (propKey.slice(0, 1) !== `$` &&
                properties[propKey] !== undefined &&
                properties[propKey] !== null) {
                lodash_set(variancePayload, `properties.${propKey}`, properties[propKey]);
            }
        });
    }
    ...
    Object.keys(mapping).forEach((varianceKeyPath) => {
        ...
        if (Array.isArray(pHKeyPath)) {
            for (let i = 0; i < pHKeyPath.length; i++) {
                pHKeyVal = lodash_get(inPayload, pHKeyPath[i]);
                if (pHKeyVal) {
                    break;
                }
            }
            ...
            pHKeyVal = lodash_get(inPayload, pHKeyPath);
        }
        if (pHKeyVal !== undefined && pHKeyVal !== null) {
            lodash_set(outPayload, varianceKeyPath, pHKeyVal);
        }
    });
}
